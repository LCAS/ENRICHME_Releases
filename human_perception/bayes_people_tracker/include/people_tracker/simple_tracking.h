...
bool MTRK::initialize(FilterType* &filter, sequence_t& obsvSeq) {
    ...
    bool validObs = false;
    std::vector<observation_t>::iterator ui = obsvSeq.begin();
    while (ui != obsvSeq.end()) {

//        cout << (*ui).flag << endl;
        if ((*ui).flag.compare("upper_body_detector")==0) {
//             cout << "Found upper_body_detector" << endl;
             validObs = true;
        }
        ui++;
    }
...
public:
    ...
    void addDetectorModel(std::string name, association_t alg, double pos_noise_x, double pos_noise_y, unsigned int seqSize = 5, double seqTime = 0.2) {
        ...
        det.seqSize = seqSize;
        det.seqTime = seqTime;
        //ROS_INFO("seqSize:%i seqTime: %f", seqSize, seqTime);
...
//        mtrk.process(*(det.ctm), det.alg);
        ...
        for (li = obsv.begin(); li != liEnd; li++) {
            (*observation)[0] = li->x;
            (*observation)[1] = li->y;
            mtrk.addObservation(*observation, obsv_time, detector_name);
        }
