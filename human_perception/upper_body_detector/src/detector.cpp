                            ...
                            const Matrix<double> &upper_body_template, Vector<Vector<double> > &detected_bounding_boxes)
{
    ...
    //ROS_INFO("depth map width:%i, height:%i", width,height);
...
//    Matrix<int> labeledROIs;
    ...
    //ROS_INFO("upper_body_detector: computing free space...");
    ComputeFreespace(camera_origin, labeledROIs, mat_2D_pos_x, mat_2D_pos_y, point_cloud);
    //ROS_INFO("upper_body_detector: 2D positions are computed...");

    Vector<ROI> all_ROIs;
    //ROS_INFO("upper_body_detector: preprocessing ROIs...");
    PreprocessROIs(labeledROIs, all_ROIs);
    //ROS_INFO("upper_body_detector: preprocessing done...");

//    Vector<Vector<Vector<double> > > points3D_in_ROIs(all_ROIs.getSize());
    //ROS_INFO("upper_body_detector: extracting points in ROIs...");
    ExtractPointsInROIs(/*points3D_in_ROIs*/all_ROIs, mat_2D_pos_x, mat_2D_pos_y, point_cloud, labeledROIs);
    //ROS_INFO("upper_body_detector: points are extracted...");
    ...
    //ROS_INFO("upper_body_detector: number of ROIs: %i",all_ROIs.getSize());
...
//                if( ROI::GetRegion2D(bbox, camera_origin, points3D_in_ROIs(l)) )
    ...
    //ROS_INFO("upper_body_detector: distances are calculated...");

    //ROS_INFO("upper_body_detector: number of distances: %i",distances.getSize());

    detected_bounding_boxes = EvaluateTemplate(upper_body_template, depth_map, close_range_BBoxes, distances);
    //ROS_INFO("upper_body_detector: template detection...");
}
                     ...
                     const PointCloud &point_cloud)
{
    ...
    //ROS_INFO("plane_par_0:%f, plane_par_1:%f, plane_par_2:%f, plane_par_3:%f",plane_par_0,plane_par_1,plane_par_2,plane_par_3);
//    double log_2 = log(2);
    ...
    for(int j = 0; j < point_cloud.X.getSize(); j++)
    {
        ...
        if(zj < Globals::freespace_max_depth_to_cons && zj >= 0.1)
        {
            ...
            //ROS_INFO("xj:%f, yj:%f, zj:%f, d2plane:%f",xj,yj,zj,distance_to_plane);
...
//    occ_map.WriteToTXT("before.txt");
     ...
     //ROS_INFO("getGaussian1D");
     occ_map = AncillaryMethods::conv1D(occ_map, kernel, false);
     //ROS_INFO("conv1D - false");
     occ_map = AncillaryMethods::conv1D(occ_map, kernel, true);
     //ROS_INFO("conv1D - true");
    
    //ros::Time crntTime = ros::Time::now();
    //char filename[150];
    //sprintf(filename,"/home/scosar/Documents/Datasets/ros_test/after/after_%d-%d.txt",crntTime.sec,crntTime.nsec);
    //ROS_INFO("sec:%i",crntTime.sec);
    //occ_map.WriteToTXT(filename);
 
    //ROS_INFO("occ_map_binary...");
    //ROS_INFO("x_bins:%i z_bins:%i",x_bins,z_bins);

    int occ_map_length = x_bins*z_bins;
    //ROS_INFO("occ_map_length:%d",occ_map_length);
    //ROS_INFO("occ_map_binary -> x_size :%d, occ_map_binary -> y_size :%d",occ_map_binary.x_size(),occ_map_binary.y_size());
    /*for(int i = 0; i < occ_map_length; i++)
    {
	ROS_INFO("occ_map_binary -> i:%d",i);
