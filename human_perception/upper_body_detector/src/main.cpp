                    ...
                    int r, int g, int b, int lineWidth)
{
    ...
    for(int i = 0; i < detections.pos_x.size(); i++){
        int x =(int) detections.pos_x[i]*Globals::scale_x;
        int y =(int) detections.pos_y[i]*Globals::scale_y;
        int w =(int) detections.width[i]*Globals::scale_x;
        int h =(int) detections.height[i]*Globals::scale_y;
...
bool ReadConfigParams(ros::NodeHandle n)
{
    ...
    success = checkParam(n.getParam(ns+"dRGBHeight", Globals::dRGBHeight), ns+"dRGBHeight") && success;
    success = checkParam(n.getParam(ns+"dRGBWidth", Globals::dRGBWidth), ns+"dRGBWidth") && success;

    Globals::scale_x = Globals::dRGBWidth/Globals::dImWidth;
    Globals::scale_y = Globals::dRGBHeight/Globals::dImHeight;
...
void callback(const ImageConstPtr &depth, const ImageConstPtr &color,const GroundPlane::ConstPtr &gp, const CameraInfoConstPtr &info)
{
    ...
    //ros::Time crntTime = ros::Time::now();
    //char filename[150];
    //sprintf(filename,"/home/scosar/Documents/Datasets/ros_test/matrix/matrix_%d-%d.txt",crntTime.sec,crntTime.nsec);
    //ROS_INFO("time:%i.%i",crntTime.sec,crntTime.nsec);
    //matrix_depth.WriteToTXT(filename);
    //matrix_depth.WriteToTXT("/home/scosar/matrix.txt");
    //unsigned int valushort = img_depth_.at<ushort>(211,255);
    //float valf = img_depth_.at<float>(211,255);
    //float newVal = valushort/1000.0;
    //ROS_INFO("valf:%f, valushort:%i, newVal:%f", valf,valushort,newVal);
    //ROS_INFO("upper_body_detector: depth image copied...");
    ...
    //ROS_INFO("upper_body_detector: PointCloud object created...");
