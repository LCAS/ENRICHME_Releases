...
#include <reidentifier/reidentifier_paramsConfig.h>
...
ros::Publisher pub_message, pub_head3DPt, pub_shoulderLeft3DPt, pub_shoulderRight3DPt, pub_markers, pub_features, pub_ids;
...
//    point_cloud.X.writeToTXT("/home/scosar/Documents/X_Data.txt");
    ...
    ridentify->ExtractFeatures(point_cloud, detected_bounding_boxes,false);
    int pId = ridentify->Recognize();

    // ROS_INFO("reidentifier: feature extraction done...");

    // Generate messages
    std_msgs::Int32 personID_msg;
    personID_msg.data = pId;
    ...
    // Body Points
    geometry_msgs::PoseArray head3DPtPeople, shoulderLeft3DPtPeople, shoulderRight3DPtPeople;
    head3DPtPeople.header = depth->header;
    shoulderLeft3DPtPeople.header = depth->header;
    shoulderRight3DPtPeople.header = depth->header;
    ...
    for(int jj = 0; jj < detected_bounding_boxes.getSize(); jj++)
    {
        ...
        if (ridentify->heightPeople.size()>jj)
        {
            ...
            personDataArray.data.push_back(ridentify->distances[jj]);
            ...
            //cout << "volFace: " << ridentify->volFacePeople[jj] << "volBreast: " << ridentify->volBreastPeople[jj] << "volBelly: " << ridentify->volBellyPeople[jj] << "dist: " << ridentify->distances[jj] <<  endl;


            geometry_msgs::Pose poseHead;
            poseHead.position.x = ridentify->head3DPtPeople[jj][0];
            poseHead.position.y = ridentify->head3DPtPeople[jj][1];
            poseHead.position.z = ridentify->head3DPtPeople[jj][2];
            poseHead.orientation.w = 1.0; //No rotation atm.
            head3DPtPeople.poses.push_back(poseHead);

            geometry_msgs::Pose poseShL;
            poseShL.position.x = ridentify->shoulder3DPtsPeople[jj][0][0];
            poseShL.position.y = ridentify->shoulder3DPtsPeople[jj][0][1];
            poseShL.position.z = ridentify->shoulder3DPtsPeople[jj][0][2];
            poseShL.orientation.w = 1.0; //No rotation atm.
            shoulderLeft3DPtPeople.poses.push_back(poseShL);

            geometry_msgs::Pose poseShR;
            poseShR.position.x = ridentify->shoulder3DPtsPeople[jj][1][0];
            poseShR.position.y = ridentify->shoulder3DPtsPeople[jj][1][1];
            poseShR.position.z = ridentify->shoulder3DPtsPeople[jj][1][2];
            poseShR.orientation.w = 1.0; //No rotation atm.
            shoulderRight3DPtPeople.poses.push_back(poseShR);
        }

    }

    pub_features.publish(personDataArray);
    pub_head3DPt.publish(head3DPtPeople);
    pub_shoulderLeft3DPt.publish(shoulderLeft3DPtPeople);
    pub_shoulderRight3DPt.publish(shoulderRight3DPtPeople);
...
//                cout << "height: " << heights.at(jj) << ", shoulders: " << shoulders.at(jj) << ", faces: "  << faces.at(jj) << endl;
                // ROS_INFO_STREAM("height: " << ridentify->heightPeople[jj] << ", shoulders: " << ridentify->widthShoulders[jj] << ", faces: "  << ridentify->lengthFaces[jj] << ", distance: " << ridentify->distances[jj]);
...
//                RenderSegmentedBlob(ridentify->segmentedBlobs[jj], image_depth, 250, 0, 0 );
                ...
                if (pId == jj)
                {
                    RenderBBox2DwithBiometricDistance(biometricFeatures, ridentify->distances[jj], detected_bounding_boxes(jj), image_depth, 0, 255, 0);
                    RenderBBox2DwithBiometricDistance(biometricFeatures, ridentify->distances[jj], detected_bounding_boxes(jj), image_rgb, 0, 255, 0);
                }
                else
                {
                    RenderBBox2DwithBiometricDistance(biometricFeatures, ridentify->distances[jj], detected_bounding_boxes(jj), image_depth, 0, 0, 255);
                    RenderBBox2DwithBiometricDistance(biometricFeatures, ridentify->distances[jj], detected_bounding_boxes(jj), image_rgb, 0, 0, 255);
                }
...
//        pub_result_image.publish(cv_depthdisplay_ptr->toImageMsg());
    ...
    pub_message.publish(personID_msg);


}


void matchCallback(const UpperBodyDetector::ConstPtr &detections, const bayes_people_tracker::PeopleTracker::ConstPtr &people, const CameraInfoConstPtr &info)
{
...
//    Camera camera(K,R,t,GP);
    ...
    Vector<Vector< double > > detected_bounding_boxes;
    Vector<double> det;
    int noofDetections = detections->pos_x.size();

//    ROS_INFO("matchCallback - size detections:%i", noofDetections);

    Vector<int> matchedIDs(noofDetections,0.0);
    for (int i=0; i<noofDetections; i++)
    {
        det.clearContent();
        det.pushBack(detections->pos_x[i]);
        det.pushBack(detections->pos_y[i]);
        det.pushBack(1.0);

        detected_bounding_boxes.pushBack(det);

        int noofPeople = people->poses.size();
//        ROS_INFO("matchCallback - size detections:%i", noofPeople);

        Vector<double> distToPositions;
        Vector<Vector< double> > pixels_kinect;
        double minDist = 10000;
        for (int j=0; j<noofPeople; j++)
        {

            geometry_msgs::PoseStamped pose,pose_kinect;
            pose.pose = people->poses[j];
            pose.header = people->header;

//            ROS_INFO_STREAM("waitForTransform... from " << people->header.frame_id << " to " << detections->header.frame_id);
//            listener.waitForTransform(people->header.frame_id,detections->header.frame_id,ros::Time::now(),ros::Duration(2.0));
            ROS_INFO("transformPose...");
            listener.transformPose(detections->header.frame_id,pose,pose_kinect);
//            listener.transformPose("/k2/depth_frame",pose,pose_kinect);


            Vector<double> pt_kinect(3,0.0);
            pt_kinect[0] = pose_kinect.pose.position.x / pose_kinect.pose.position.z;
            pt_kinect[1] = -0.4;//pose_kinect.pose.position.y;
            pt_kinect[2] = 1.0;

            Vector<double> pix_kinect(3,0.0);

            pix_kinect = K*pt_kinect;
//            pix_kinect.show();
//            pix_kinect.writeToTXT("/home/scosar/Documents/pix_kinect.txt");

            pixels_kinect.pushBack(pix_kinect);

            Vector<double> dif = det - pix_kinect;
            double dist = dif.norm();
...
//    matchedIDs.show();
//    matchedIDs.writeToTXT("/home/scosar/Documents/matchedIDs.txt");


    std_msgs::Int32MultiArray matchedIDArray;
    //Clear array
    matchedIDArray.data.clear();
    std_msgs::MultiArrayLayout arrayLayout;
    std_msgs::MultiArrayDimension arrayDim1;
    arrayDim1.label = "IDs";
    arrayDim1.size = detected_bounding_boxes.getSize();
    arrayDim1.stride = detected_bounding_boxes.getSize();

    arrayLayout.dim.push_back(arrayDim1);
    arrayLayout.data_offset = 0;
    matchedIDArray.layout = arrayLayout;


    for(int jj = 0; jj < detected_bounding_boxes.getSize(); jj++)
    {

        matchedIDArray.data.push_back(matchedIDs[jj]);
    }

    pub_ids.publish(matchedIDArray);

    // ROS_INFO("reidentifier: processing frame...");
    // ROS_INFO("callback - size bbox:%i", detected_bounding_boxes.getSize());
    ridentify->SetIDs(matchedIDs);
...
void connectCallback(message_filters::Subscriber<CameraInfo> &sub_cam,
                     ...
                     message_filters::Subscriber<bayes_people_tracker::PeopleTracker> &sub_people,
    ...
    if(!pub_message.getNumSubscribers()
        ...
        sub_gp.subscribe();
        sub_det.subscribe();
        sub_people.subscribe();
        sub_dep.subscribe(it,sub_dep.getTopic().c_str(),1);
        sub_col.subscribe(it,sub_col.getTopic().c_str(),1);
    }
}

void configCallback(reidentifier::reidentifier_paramsConfig &config, uint32_t level)
{
  // Set class variables to new values. They should match what is input at the dynamic reconfigure GUI.
  ridentify->SetThreshold(config.reidentify_threshold);
  // end configCallback()
}
...
//    vm = new VisualisationMarkers();
    ...
    string pub_topic_head3DPt;
    string pub_topic_shoulderLeft3DPt;
    string pub_topic_shoulderRight3DPt;
    string pub_topic_ri;
    string pub_topic_matchedIDs;
    ...
    private_node_handle_.param("people_tracker", topic_people, string("/people_tracker/positions"));
    ...
    message_filters::Subscriber<bayes_people_tracker::PeopleTracker> subscriber_people(n, topic_people.c_str(), 1); subscriber_people.unsubscribe();
    ...
    // private_node_handle_.param("reidentify_threshold", ridentify_thrs, float(1.5));
    // ridentify->SetThreshold(ridentify_thrs);
    ridentify->readLearnedModel(personName,meanVector,meanVector_rows,meanVector_cols,icovMatrix,icovMatrix_rows,icovMatrix_cols);

    // Set up a dynamic reconfigure server.
    // This should be done before reading parameter server values.
    dynamic_reconfigure::Server<reidentifier::reidentifier_paramsConfig> dr_srv;
    dynamic_reconfigure::Server<reidentifier::reidentifier_paramsConfig>::CallbackType cb;
    ...
    ///////  Matching ID  //////////
//    // For matching IDs: The real queue size for synchronisation is set here.
//    sync_policies::ApproximateTime<UpperBodyDetector, bayes_people_tracker::PeopleTracker, CameraInfo> MySyncPolicy_matching(queue_size);
//    MySyncPolicy_matching.setAgePenalty(1000); //set high age penalty to publish older data faster even if it might not be correctly synchronized.

//     // For matching IDs: Create synchronization policy. Here: async because time stamps will never match exactly
//    const sync_policies::ApproximateTime<UpperBodyDetector, bayes_people_tracker::PeopleTracker, CameraInfo> MyConstSyncPolicy_matching = MySyncPolicy_matching;
//    Synchronizer< sync_policies::ApproximateTime<UpperBodyDetector, bayes_people_tracker::PeopleTracker, CameraInfo> > sync_matching(MyConstSyncPolicy_matching,
//                                                                                               subscriber_det,
//                                                                                               subscriber_people,
//                                                                                               subscriber_camera_info);


//    // For matching IDs: Register one callback for all topics
//    sync_matching.registerCallback(boost::bind(&matchCallback, _1, _2, _3));

    ///////  Matching ID  //////////

    // Create publisher
    private_node_handle_.param("reidentifier_result", pub_topic_ri, string("/reidentifier/recognizedID"));
    pub_message = n.advertise<std_msgs::Int32>(pub_topic_ri.c_str(), 10, con_cb, con_cb);
    ...
    private_node_handle_.param("reidentifier_head3DPt", pub_topic_head3DPt, string("/reidentifier/head3DPt"));
    pub_head3DPt = n.advertise<geometry_msgs::PoseArray>(pub_topic_head3DPt.c_str(), 10, con_cb, con_cb);

    private_node_handle_.param("reidentifier_shL3DPt", pub_topic_shoulderLeft3DPt, string("/reidentifier/shL3DPt"));
    pub_shoulderLeft3DPt = n.advertise<geometry_msgs::PoseArray>(pub_topic_shoulderLeft3DPt.c_str(), 10, con_cb, con_cb);

    private_node_handle_.param("reidentifier_shR3DPt", pub_topic_shoulderRight3DPt, string("/reidentifier/shR3DPt"));
    pub_shoulderRight3DPt = n.advertise<geometry_msgs::PoseArray>(pub_topic_shoulderRight3DPt.c_str(), 10, con_cb, con_cb);

    private_node_handle_.param("reidentifier_matchedIDs", pub_topic_matchedIDs, string("/reidentifier/matchedIDs"));
    pub_ids = n.advertise<std_msgs::Int32MultiArray>(pub_topic_matchedIDs.c_str(), 10, con_cb, con_cb);
